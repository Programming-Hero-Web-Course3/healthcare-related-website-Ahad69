...
const SignIn = () => {
     ...
     const {handleRegister, 
        ...
        const handle = () =>{
            handleRegister()
            .then(result =>{
                console.log("ahad" , result)
                history.push(redirect)
            })
        }
    return (
        <div className="login-form">
            {isLogin ? <form onSubmit={handle}>
            <div className="m-5">
            <h1>Login</h1>
        ...
        </form>: <form onSubmit={handle}>
            <div className="m-5">
                <h1>Registration</h1>
                <input  className="input m-3 p-2 w-25 rounded-pill" type="text" placeholder="First Name" required/>
                <br />
                <input  className="input m-3 p-2 w-25 rounded-pill" type="text" placeholder="Last Name" required/>
                <br />
                <input onBlur={emailHandler} className="input m-3 p-2 w-25 rounded-pill" type="email" placeholder="Email" required/>
                <br />
                <input onBlur={passwordHandler} className="input m-3 p-2 w-25 rounded-pill" type="password" placeholder="Password" required/>
